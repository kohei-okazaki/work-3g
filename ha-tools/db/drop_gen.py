...
def generate(tables: dict[str, str]):
    ...
    # テンプレートファイルを取得
    template = utils.get_template(const.DROP_TEMPLATE_FILE)

    # 出力先ディレクトリ作成
    output_path = Path(const.DROP_OUTPUT_PATH)
    output_path.mkdir(exist_ok=True)

    # Drop SQL生成
    drop_sql = template.render(tables=tables)

    (output_path / const.DROP_FILE_NAME).write_text(drop_sql, encoding="utf-8")
