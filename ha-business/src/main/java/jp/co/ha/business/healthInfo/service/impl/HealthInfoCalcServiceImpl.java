...
public class HealthInfoCalcServiceImpl implements HealthInfoCalcService {
    ...
    public BigDecimal calcBmi(BigDecimal height, BigDecimal weight, int digit) {
        BmiCalcFunction function = (h, w, d) -> {
            // height * height
            BigDecimal multiplyResult = Calculator.calc(h, CalcMethod.MULTIPLY, h, d,
                    RoundingMode.HALF_UP);
            // weight / multiplyResult
            BigDecimal result = Calculator.calc(w, CalcMethod.DIVIDE, multiplyResult, d,
                    RoundingMode.HALF_UP);
            return result;
        };
    ...
    public BigDecimal calcStandardWeight(BigDecimal height, int digit) {
        StandardWeightCalcFunction function = (h, d) -> {
            // height * height
            BigDecimal result = Calculator.calc(h, CalcMethod.MULTIPLY, h, d,
                    RoundingMode.HALF_UP);
            // result * 22
            return Calculator.calc(result, CalcMethod.MULTIPLY, new BigDecimal(22), d,
                    RoundingMode.HALF_UP);
        };
    ...
    public BigDecimal calcBaseMetabolism(CalorieCalcDto calorieCalcDto) {
        ...
        result = Calculator.calc(calcWeight, CalcMethod.ADD, calcHeight, 3,
                RoundingMode.HALF_UP);
        result = Calculator.calc(result, CalcMethod.SUBTRACT, calcAge, 3,
                RoundingMode.HALF_UP);
        result = Calculator.calc(result, CalcMethod.ADD, adjustValue, 3,
                RoundingMode.HALF_UP);
