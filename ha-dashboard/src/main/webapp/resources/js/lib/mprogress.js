(function() {
  ...
  var $$utils$$Utils = {
    ...
    /**
     * Applies css properties to an element, similar to the jQuery setcss
     * method.
     * 
    ...
    /**
     * converts a percentage (`0..1`) to a bar translateX percentage
     * (`-100%..0%`).
     */
    ...
    hasClass : function(element, name) {
      var list = typeof element == 'string' ? element : $$utils$$Utils
          .classList(element);
      return list.indexOf(' ' + name + ' ') >= 0;
    },

    addClass : function(element, name) {
      var oldList = $$utils$$Utils.classList(element), newList = oldList + name;

      if ($$utils$$Utils.hasClass(oldList, name))
        return;
    ...
    removeClass : function(element, name) {
      var oldList = $$utils$$Utils.classList(element), newList;

      if (!$$utils$$Utils.hasClass(element, name))
        return;
      ...
      (
          ...
          function() {
            ...
            MProgress.prototype = {
              ...
              /**
               * Shows the progress bar. This is the same as setting the status
               * to 0%, except that it doesn't go backwards.
               * 
               * MProgress.start();
               * 
               */
              start : function() {
                if (!this.status && !this._isBufferStyle())
                  this.set(0);
               ...
               * setting the status to 100%, with the difference being `end()`
              ...
              /**
               * Sets the progress bar status, where `n` is a number from `0.0`
               * to `1.0`.
               * 
               * MProgress.set(0.4); MProgress.set(1.0);
               */
              set : function(n) {
              ...
              _render : function(noFromStart) {
                ...
                if (!this._isIndeterminateStyle() && !this._isQueryStyle()) {
                  ...
                  var bar = progress.querySelector(this._getCurrSelector());
                  var perc = fromStart ? '-100' : $$utils$$default
                      .toBarPerc(this.status || 0);
                  ...
                  if (this._isBufferStyle()) {
                    var buffer = progress.querySelector(SELECTOR_BUFFER), bufferPerc = fromStart ? '-100'
                        : $$utils$$default.toBarPerc(this.bufferStatus || 0);
               ...
               */
              _remove : function() {
                var progress = this._getRenderedId(), MParent = document
                ...
                if (MParent != document.body) {
                  $$utils$$default.removeClass(MParent,
              ...
              _setProgress : function(barSelector, n) {
                ...
                $$utils$$default.queue(function(next) {
                  ...
                  // Add transition
                  $$utils$$default.setcss(bar, that._barPositionCSS(n, speed,
                      ease));
              ...
              _getRandomNum : function(n, amount) {
                if (typeof amount !== 'number') {
                  amount = (1 - n)
                      * $$utils$$default.clamp(Math.random() * n, 0.1, 0.95);
                }
              ...
              _getRenderedId : function(getId) {
                ...
                var parentFiltered = this.options.parent.match(
                    /([a-zA-Z0-9]+)/ig).join("-");
              ...
              _getCurrTemplate : function() {
                var tplType = this.options.template || 1, tplNameArr = [
                    'determinate', 'buffer', 'indeterminate', 'query' ], tplKey;
              ...
              _barPositionCSS : function(n, speed, ease) {
                ...
                if (this.options.positionUsing === 'translate3d') {
                  barCSS = {
                    transform : 'translate3d(' + $$utils$$default.toBarPerc(n)
                        + '%,0,0)'
                  };
                } else if (this.options.positionUsing === 'translate') {
                  barCSS = {
                    transform : 'translate(' + $$utils$$default.toBarPerc(n)
                        + '%,0)'
                  };
                } else {
                  barCSS = {
                    'margin-left' : $$utils$$default.toBarPerc(n) + '%'
                  };
                }
            ...
            /**
             * Waits for all supplied jQuery or Zepto promises and increases the
             * progress as the promises resolve.
             * 
             * @param $promise
             *          jQuery or Zepto Promise
             */
            (function() {
